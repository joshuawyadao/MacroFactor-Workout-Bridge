...
### Automatic mode (0.8.0)

Open the app with **Auto-load inboxes** enabled. It discovers `local-data` beside the installed project, or remembers the folder you select with **Workspace…**. It loads the local exercise mapping when present and the existing `annotations/workout-history.json`. You no longer need to choose an export on every launch. A remembered folder that disappears is an error, not permission to switch datasets.
...
Version 0.7.0 opens on **Dashboard**. Main-lift cards show exact variations in consistent cyan/lavender/amber colors. Scroll horizontally through block report cards and click **Analyze block** to focus **Training timeline**, or click **Explore** on a lift for the all-block timeline. The timeline aligns three lift charts and all-exercise weekly workload, with saved-context markers that can be toggled. Click a chart week, or use the accessible week/exercise controls, to inspect the original logged sets and source rows. **← Overview** returns to the dashboard. Both views share range and variation choices. They scroll at smaller window sizes.

Block report averages use full Monday–Sunday weeks inside both the chosen range and the export's observed date bounds; the denominator is shown on each card. Partial weeks remain visible but are excluded from averages. Interior no-log weeks count as zero logged workload, not confirmed skips. Partial/outside-range and invalidly dated blocks are labelled; no unavailable average is replaced by zero. **Show exact block values** exposes best estimates and weekly averages in a table. **Training focus** shows sets/week by exercise, not muscle group: no unverified muscle attribution or hard-set assumption is applied. The context panel quotes saved notes and does not infer causes from load changes.
