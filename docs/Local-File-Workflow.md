...
### Automatic mode (0.9.0)

Open the app with **Data → Auto-load inboxes** enabled. It discovers `local-data` beside the installed project, or remembers the folder you select with **Data → Choose workspace…**. It loads the local exercise mapping when present and the existing `annotations/workout-history.json`. You no longer need to choose an export on every launch. A remembered folder that disappears is an error, not permission to switch datasets.

Drop new exports in the corresponding inbox. On startup and after a managed-file change (checked every 30 seconds), background ingestion archives new content without modifying originals. Already-archived hashes are reused and unchanged refreshes create no empty manifests. The archive/current-link conventions above remain unchanged; dashboard selection is separate from the narrower current export used by Weekly Bridge. **Refresh inboxes** also checks immediately. No MacroFactor API, download automation, or arbitrary Downloads-folder scan is involved.
...
**Weekly feedback →** opens the export's latest mapped Monday–Sunday week. Save the existing private annotation form. Background refresh pauses for unsaved feedback and checks again after saving. Explicit refresh can discard edits only after a default-No confirmation. If the annotation file changed externally, save is refused while your text remains available to copy. Invalid/newer files or a missing workspace leave previously loaded data visible with an explicit stale-data warning. All data stays local. To deliberately select one export instead, disable automatic mode; the standard manual controls become available.

### Dashboard views and manual selection

Version 0.9.0 reduces default controls: **Weekly feedback** stays in the compact toolbar, maintenance commands are under **Data**, and the clickable data indicator shows latest coverage and any conflict/notice counts. Click it for full source status and analysis warnings. Errors and refresh-paused notices remain visible. Use **Data → Select files manually…** to reveal manual inputs; enable **Auto-load inboxes** again to return to automatic mode. **More** contains Block details, Compare two blocks and Block context; a selected secondary view stays visibly identified until you leave it. The simpler presentation does not change calculations or stored files, and does not replace the installed app automatically.

The app opens on **Dashboard**. Main-lift cards show exact variations in consistent cyan/lavender/amber colors. Scroll horizontally through block cards and click a block heading to focus **Training timeline**, or a lift heading for the all-block timeline. Headings and exercise links replace repeated boxed action buttons, with keyboard access and visible focus retained. The timeline aligns three lift charts and all-exercise weekly workload, with saved-context markers that can be toggled. Click a chart week, or use the accessible week/exercise controls, to inspect the original logged sets and source rows. **← Overview** returns to the dashboard. Both views share range and variation choices. They scroll at smaller window sizes.
...
**Explore exercise** retains name search, family filters, chart metrics, and its independent all-history/4/12/24-week ranges anchored to the export's last workout week. Accessory workload buttons open this view. Earlier gaps and unmapped weeks remain visible; short exports prompt you to select all-time history. The old overview remains **Block details**, and the relative-week comparison is **Compare two blocks**.
...
The stable MacroFactor file under `current/` is selected for the weekly bridge: when exports end on the same workout date, the narrower later-starting export wins. It therefore may not contain all-time history. Automatic History mode consolidates managed snapshots independently; in manual mode choose the all-time inbox or archive file explicitly.
