...
def _quiet_button(text, accessible_name=""):
...
class LiftCard(QFrame):
    ...
    def __init__(self, family):
        ...
        outer.setSpacing(4)
        self.button = _quiet_button(f"{family} ↗", f"Explore {family.lower()} timeline")
        self.button.setToolTip(f"Open the {family.lower()} training timeline")
        self.button.setStyleSheet(f"color: {LIFT_COLORS[family]}; font-size: 18px; font-weight: 600; text-align: left;")
        self.button.clicked.connect(lambda: self.explore.emit(self.exercise.currentText()))
        outer.addWidget(self.button)
        self.exercise = QComboBox()
        self.exercise.setObjectName("quietSelector")
        self.exercise.setAccessibleName(f"{family} variation")
        self.exercise.setToolTip("Choose an exact variation; different movements are not combined.")
    ...
    def _refresh(self, *_args):
        ...
        if self.dashboard and self.exercise.currentText():
            ...
            if logged:
                ...
                self.detail.setText(f"Est. 1RM · week of {latest.start:%b %d, %Y}")
                self.detail.setToolTip("Estimated 1RM in pounds from the last logged week in this range. Not a tested maximum. Missing weeks are not zero.")
            else:
...
class HistoryHome(QScrollArea):
    ...
    def __init__(self):
        ...
        self.coverage = _label("Load workout history to view progress.")
        self.coverage.setObjectName("subtitle")
        ...
        title = _label("Across your blocks")
        title.setObjectName("sectionTitle")
        title.setToolTip("Scroll horizontally to browse blocks. Select a block heading for its training timeline.")
        outer.addWidget(title)
        self.report_scroll = QScrollArea()
        self.report_scroll.setWidgetResizable(True)
        self.report_scroll.setFixedHeight(180)
        ...
        outer.addWidget(self.workload_frame)
        self.values_button = _quiet_button("Notes && exact values ▸", "Show notes, methodology and exact block values")
        self.values_button.setCheckable(True)
        outer.addWidget(self.values_button, alignment=Qt.AlignmentFlag.AlignLeft)
        self.details_panel = QFrame()
        self.details_panel.setObjectName("summaryCard")
        context_layout = QVBoxLayout(self.details_panel)
        context_title = _label("Saved context")
        context_title.setObjectName("sectionTitle")
        context_layout.addWidget(context_title)
        self.context = _label()
        context_layout.addWidget(self.context)
        note = _label("Saved notes only. Lower loads do not automatically mean fatigue or lost strength. Exercise workload describes logged sets, not muscle growth.")
        note.setObjectName("subtitle")
        context_layout.addWidget(note)
        context_layout.addWidget(self.averages_note)
        ...
        context_layout.addWidget(self.legend)
        ...
        context_layout.addWidget(self.blocks)
        outer.addWidget(self.details_panel)
        self.details_panel.hide()
        self.values_button.toggled.connect(self.details_panel.setVisible)
        self.values_button.toggled.connect(lambda expanded: self.values_button.setText("Notes && exact values ▾" if expanded else "Notes && exact values ▸"))
    ...
    def set_history(self, dashboard, annotations=None):
        ...
        if dashboard:
            ...
            self.coverage.setText("Load workout history to view progress.")
    ...
    def _refresh_blocks(self):
        ...
        for row, report in enumerate(reports):
            ...
            layout.setSpacing(4)
            title = _quiet_button(block.name, f"Analyze block {block.name}")
            title.setStyleSheet("font-weight: 600; text-align: left;")
            title.ensurePolished()
            title.setText(title.fontMetrics().elidedText(block.name, Qt.TextElideMode.ElideRight, 214) + " ↗")
            title.setToolTip(block.name)
            title.setEnabled(bool(report.weeks) and not report.issue)
            title.clicked.connect(lambda checked=False, name=block.name: self.focus_block.emit(name))
            layout.addWidget(title)
            date_and_type = _label(f"{block.start_date or 'Not dated'} · {option_label(BLOCK_TYPE_OPTIONS, block.block_type)}")
            date_and_type.setObjectName("subtitle")
            layout.addWidget(date_and_type)
            metric = QHBoxLayout()
            value = _label(decimal_text(report.sets_per_week))
            value.setObjectName("summaryValue")
            value.setAccessibleName(f"{decimal_text(report.sets_per_week)} sets per week")
            metric.addWidget(value)
            metric.addWidget(_label("sets / week"), 1, Qt.AlignmentFlag.AlignBottom)
            layout.addLayout(metric)
            denominator = _label(f"{decimal_text(report.days_per_week)} days / week · {report.complete_week_count} full weeks")
            denominator.setToolTip(f"Averages use {report.complete_week_count} full weeks from this {len(block.week_labels)}-week block. Partial weeks are excluded; no logs do not confirm a skipped workout.")
        ...
        title = _label("Training focus")
        title.setObjectName("sectionTitle")
        self.workload_layout.addWidget(title)
        subtitle = _label(f"Sets / week · {len(full_weeks(self.dashboard, weeks))} full weeks")
        subtitle.setObjectName("subtitle")
        subtitle.setToolTip("Average logged sets by exercise, not muscle growth. Partial weeks are excluded.")
        self.workload_layout.addWidget(subtitle)
        workload = exercise_workload(self.dashboard, weeks)[:5]
        for name, value in workload:
            button = _quiet_button(f"{name}   {decimal_text(value)} ↗", f"Explore {name}, {decimal_text(value)} sets per week")
