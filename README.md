...
Version 0.6.0 opens on the Workout History dashboard. The desktop interface uses black/charcoal surfaces, white text, high-contrast primary actions, visible keyboard focus, and distinct selected/disabled controls. The theme is currently dark-only, independent of the system appearance. Yellow `Skip` review markers retain dark text for readability.

The **Dashboard** shows squat, bench, and deadlift cards with weekly estimated-1RM charts and a chronological cross-block table. Each card defaults to its most recently logged supported variation and names it explicitly; use its dropdown to choose another. Family lists are navigation groups, not combined strength metrics. Each headline is the estimate from the latest logged week for that variation, not necessarily the latest export week. The table shows each selected variation's best estimate within each block, plus whole-block logged days/sets. Blank estimates are unavailable, not zero; block totals are not completion rates or rankings, and block lengths/coverage can differ. Reduced loads do not imply fatigue.

Click a card's **Explore** button, or open **Explore exercise** directly. Search any exercise, optionally narrow to a main-lift family, and choose all history or the last 4/12/24 weeks. No block selection is required. Ranges cover whole Monday–Sunday weeks ending in the export's latest week, not today's date. The chart offers estimated 1RM, top weight, sets, or training days; shaded calendar bands and the table show block context automatically. Missing weeks remain gaps, unmapped weeks stay visible, and boundary weeks flag partial export coverage. Hover the chart for dates/values and table cells for full saved context. Named variations are never silently combined.
